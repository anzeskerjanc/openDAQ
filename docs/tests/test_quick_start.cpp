...
TEST_F(QuickStartTest, QuickStartAppReader)
{
    ...
    StreamReaderPtr reader = daq::StreamReader<double, uint64_t>(device.getSignals(SearchParams(false, true))[0]);
...
TEST_F(QuickStartTest, QuickStartAppStatistics)
{
    ...
    daq::StreamReaderPtr reader2 = daq::StreamReader<double, uint64_t>(statistics.getSignals(SearchParams(false, true))[0]);
    double amplStep = 0.1;
    double samples[100];
