...
TEST_F(ArchitectureTest, DataRule)
{
    ...
    DataRulePtr rule = device.getSignals(SearchParams(false, true))[1].getDescriptor().getRule();
...
TEST_F(ArchitectureTest, Readers)
{
    ...
    PacketReaderPtr packetReader = PacketReader(statistics.getSignals(SearchParams(false, true))[0]);
    StreamReaderPtr streamReader = StreamReader<double, uint64_t>(statistics.getSignals(SearchParams(false, true))[0]);
