...
TEST_F(TmsIntegrationTest, GetSignals)
{
    ...
    ASSERT_EQ(signals.getCount(), device.getSignals(SearchParams(true, true)).getCount());
...
TEST_F(TmsIntegrationTest, GetChannels)
{
    ...
    ASSERT_EQ(instance.getChannels(SearchParams(true, true)).getCount(), 4u);
...
TEST_F(TmsIntegrationTest, GetDomainSignal)
{
    ...
    ListPtr<ISignal> signals = clientDevice.getSignals(SearchParams(true, true));
